...
#
# def generate_channel_traffic_matrix(num_nodes, num_channels, M, channel_number):
...
#     return channel_traffic_matrix
#
# # Get user input
# M_value = 4
# channel_number = 5
#
# # Generate channel traffic matrix for the specified channel
# channel_matrix = generate_channel_traffic_matrix(len(network.nodes), 10, M_value, channel_number)
#
#
# # Use Seaborn to visualize the channel traffic matrix
# sns.set(font_scale=1.2)
# plt.figure(figsize=(10, 8))
# ax = sns.heatmap(channel_matrix, annot=True, fmt=".0f", cmap="viridis", cbar_kws={'label': 'Bit Rate (Gbps)'})
# ax.set_xticklabels(network.nodes.keys())
# ax.set_yticklabels(network.nodes.keys())
# plt.title(f"Channel Traffic Matrix (M={M_value}, Channel-{channel_number})")
# plt.show()
...
# # Assuming 'network' is an instance of your Network class
#
# def generate_channel_traffic_matrix(num_nodes, num_channels, M):
...
#
# def update_traffic_matrix(traffic_matrix, input_node, output_node, channel, R_b_gbps):
...
#
# # Generate channel traffic matrices for each channel
# channel_matrices = generate_channel_traffic_matrix(len(network.nodes), 10, M_value)
#
# # Create a mapping between node labels and integer indices
# node_indices = {node_label: i for i, node_label in enumerate(network.nodes.keys())}
#
# # Choose random input and output nodes
# input_node_label = random.choice(list(network.nodes.keys()))
# output_node_label = random.choice(list(network.nodes.keys()))
# # Choose random channel, bit rate
# random_channel = 3
# R_b_gbps = 90
# print(input_node_label)
# print(output_node_label)
# print(channel_matrices[random_channel][node_indices[input_node_label], node_indices[output_node_label]])
#
#
#
# # Update the traffic matrix for the random channel
# channel_matrices[random_channel] = update_traffic_matrix(channel_matrices[random_channel - 1],
#                                       input_node_label, output_node_label, random_channel, R_b_gbps)
...
ax = sns.heatmap(channel_matrices[selected_channel_show], annot=True, fmt=".0f", cmap="viridis", cbar_kws={'label': 'Bit Rate (Gbps)'})
ax.set_xticklabels(network.nodes.keys())
ax.set_yticklabels(network.nodes.keys())
plt.title(f"Updated Channel Traffic Matrix (M={M_value}, Channel-{selected_channel_show})")
plt.show()
