...
if __name__ == '__main__':
    ...
    three_scenarios_network = ['nodes.json', 'nodes_full.json', 'nodes_not_full.json', 'new config with 10 ch random.json' ]
...
def plot_final_traffic_matrix(req_matrices, network, M_value):
    ...
    plt.title(f"Updated Traffic Matrix (M={M_value})")
    plt.show()
def update(frame):
    ...
    if all(strength == 10 for strength in edge_strength.values()):
        ...
        print('Number of Nones',num_zero_snr)
        plt.figure()
        sns.histplot(snrs_without_zeros, bins=20, color='orange', alpha=0.7)

        # Add labels and title
        plt.xlabel('GSNR Values')
        plt.ylabel('Frequency')
        plt.title('Distribution of GSNR Values ')
        plt.show()
...
plt.title(f"Initial Traffic Matrix (M={M_value})")
print(network.nodes.keys())
