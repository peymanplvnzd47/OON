...
class Network:
    ...
    def calculate_bit_rate(self, path, strategy, GSNR, Rs=32e9, Bn=12.5e9, BERt=1e-3):

        GSNR_lin = 10 ** (GSNR / 10)
        # Check the transceiver strategy
        if strategy == 'fixed-rate':
            # Equation (1)
            Rb = 100 if GSNR_lin > 2 * erfcinv(2 * BERt ** 2) * Rs / Bn else 0
        ...
        for connection in connections:
            ...
            if best_path:
                ...
                if all_nodes_valid:
                    ...
                    calculated_bit_rate = self.calculate_bit_rate(best_path, transceiver_strategy, total_snr_db)
